...
import com.vip.user.external.services.RatingClientService;
...
public class UserServiceImpl implements UserService {
	...
	@Autowired
	RatingClientService ratingService;
	...
	public List<User> getAllUser() {
		...
		List<User> users = allUsers.stream().map(user -> {
			 ...
			 */
			List<Ratings> ratingsByUser = ratingService.getRatings(user.getUserId());
	...
	public User getUser(String userId) {
		...
		if(findById.isPresent()) {
			...
			List<Ratings> ratingsByUser = ratingService.getRatings(userId);
			List<Ratings> list = ratingsByUser.stream().map(rating ->{
					/*
					 * ResponseEntity<Hotel> entity =
					 * restTemplate.getForEntity("http://HOTEL-SERVICE/hotels/getHotel/"+rating.
